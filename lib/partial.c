...
void default_callback(ZipInfo* info, CDFile* file, size_t progress) {
    int percentDone = progress * 100/file->compressedSize;
    dprintf("Getting: %d%%", percentDone);
}
...
int download_file_from_zip(const char* url, const char* path, const char* output, PartialZipProgressCallback progressCallback) {
    ...
    if (!info) {
        dprintf("Cannot find %s.", url);
        return -1;
    }
    
    file = PartialZipFindFile(info, path);
    if (!file) {
        dprintf("Cannot find %s in %s.", path, url);
        return -1;
    }
    ...
    if(progressCallback != NULL) {
        ...
        PartialZipSetProgressCallback(info, default_callback);
    }
    
    data = PartialZipGetFile(info, file);
    if(!data) {
        dprintf("Cannot get %s from %s.", path, url);
        return -1;
    }
    size = file->size;
    ...
    fd = fopen(output, "wb");
    if(!fd) {
        dprintf("Cannot open file %s for output.", output);
        return -1;
    }
    
    if(fwrite(data, 1, size, fd) != size) {
        dprintf("Unable to write entire file to output.");
        fclose(fd);
        return -1;
    }
...
static size_t receiveData(void* data, size_t size, size_t nmemb, void** pFileData) {
	...
	if(info && info->progressCallback && file && progress) {
